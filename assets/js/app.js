...
let [$lasActiveTabBtn] = $tabBtns;
let [$lasActiveTabPanel] = $tabPanels;


addEventOnElements($tabBtns, "click", function () {
    $lasActiveTabBtn.setAttribute("aria-selected", "false");
    $lasActiveTabPanel.setAttribute("hidden", "");
    ...
    $lasActiveTabPanel = $currentTabPanel;
});
...
let apiUrl = `https://api.github.com/users/hessennasser`;
...
`.repeat(6);
    ...
    fetchData(profileUrl, data => {
        ...
        $profileCard.innerHTML = `
        <figure class="${type === "user" ? "avatar-circle" : "avatar-rounded"} img-holder" style="--width: 280;--height: 280px">
            <img src="${avatar_url}" width="280px" height="280px" alt="${username}"
                class="img-cover">
        </figure>
        ...
        ${name ? `<p class="bio">${bio}</p>` : ""
            }

        <a href="${githubPage}" target="_blank" class="btn btn-secondary"><span class="material-symbols-rounded"
                aria-hidden="true">open_in_new</span>See on Github</a>
        ...
        <div class="footer">
            <p class="copyright">&copy; 2023 smartdev</p>
        </div>
        `;
...
const $forkedPanel = document.querySelector("[data-forked-panel]");
...
const $followerTabBtn = document.querySelector("[data-followers-tab-btn]");
    ...
    fetchData(followerUrl, function (data) {
        ...
        if (data.length) {
            ...
            <figure class="avatar-circle img-holder">
                <img src="${avatar_url}&d=64" width="56" height="56" alt="${username}"
                    class="img-cover">
            </figure>
